...
def process_ts2vec_train_data(dataset_module: Any):
    train_data_numpy = dataset_module.train_data_samples.to_numpy().astype(np.float64)[..., np.newaxis]
    return train_data_numpy


def process_mcl_train_data(dataset_module: Any):
    return dataset_module.train_dataloader(strict_batch_size=True)


def get_teacher_train_data_process_fn(teacher_model: str):
    teacher_model_train_data_process_fn_map = {
        'ts2vec': process_ts2vec_train_data,
        'mcl': lambda x: x,
    }

    return teacher_model_train_data_process_fn_map[teacher_model]


def get_teacher_model_params(teacher_model: str,
                             input_shape: tuple[int, int, int],
                             recurrent_layers: list[int],
                             device: str):
    ts2vec_params = {'input_dims': input_shape[-1],
                     'hidden_dims': recurrent_layers[0],
                     'output_dims': recurrent_layers[-1],
                     'batch_size': input_shape[0],
                     'depth': 10,
                     'device': device}

    mcl_params = {'n_in': input_shape[1],
                  'output_dims': recurrent_layers[-1],
                  'batch_size': input_shape[0],
                  'device': device,
                  'alpha': 1.0,
    ...
    teacher_model_params_map = {
        'ts2vec': ts2vec_params,
        'mcl': mcl_params
    }

    return teacher_model_params_map[teacher_model]


def get_student_model_params(student_model: str,
                             input_shape: tuple[int, int, int],
                             recurrent_layers: list[int]):
    timenet_model_params = {
        'cell_type': 'GRU',
    ...
    student_model_params_map = {
        'timenet': timenet_model_params,
        'lstmae': lstmae_model_params
    }

    return student_model_params_map[student_model]
...
def main_logic(dataset_index: int,
               ...
               teacher_model: str,
               student_model: str,
               loss_sim_coeff: float,
               loss_sim_coeff_start_value: float,
               loss_sim_coeff_increase_rate: float,
               loss_sim_coeff_val: float,
               loss_start_epoch_threshold: int,
               total_epochs: int,
               device: str,
    ...
    input_shape = (data_module_params['batch_size'], dataset_seq_length, NUM_FEATURES)
    ...
    logging.info(f"Running {dataset_name} with sequence length {dataset_module.sequence_len};"
                 f" input size {input_shape}; recurrent layers {recurrent_layers};"
                 f" validation size {data_module_params['valid_size']};")

    student_model_params = get_student_model_params(student_model=student_model,
                                                    input_shape=input_shape,
                                                    recurrent_layers=recurrent_layers)

    teacher_model_params = get_teacher_model_params(teacher_model=teacher_model,
                                                    input_shape=input_shape,
                                                    recurrent_layers=recurrent_layers,
                                                    device=device)

    tsrc_framework_params = {
        ...
        'optimizer': 'Adam',
        'lr': 1e-3,
        'reg_lambda': loss_sim_coeff,
        'reg_lambda_start_value': loss_sim_coeff_start_value,
        'reg_lambda_increase_rate': loss_sim_coeff_increase_rate,
        'val_loss_coeff': loss_sim_coeff_val,
        'loss_start_epoch_threshold': loss_start_epoch_threshold,
        'total_epochs': total_epochs,
    }

    tsrc_model = TSRC(**tsrc_framework_params)

    model = Model(name=f'tsrc-{teacher_model}-{student_model}',
                  model=tsrc_model,
                  encoders=['student_model_encoder'])

    logging.info(f'Starting to run {model.name} on {dataset_name}')

    logging.info(f'TSRC framework params: {str(tsrc_framework_params)}')

    logging.info(f'TSRC framework models description: {str(model.model)}')

    early_stopping_constants = get_early_stopping_params(max_epochs=total_epochs)
    early_stopping_constants['patience'] = total_epochs // 2
    early_stopping_constants['min_delta'] = 0.0001
    ...
    device_accelerator_map = {
        'cpu': 'cpu',
        'cuda': 'gpu'
    }

    accelerator = device_accelerator_map.get(device, 'auto')
    ...
    teacher_model_train_data = get_teacher_train_data_process_fn(teacher_model)(dataset_module)
    model.model.train_teacher_model(train_data=teacher_model_train_data)
    ...
    if best_model_path is not None and Path(best_model_path).exists():
        ...
        # make a deep copy of the final model
...
def main():
    ...
    parser.add_argument('--teacher_model',
                        type=str,
                        help='The teacher model to use in the framework',
                        default='ts2vec')
    parser.add_argument('--student_model',
                        type=str,
                        help='The student model to use in the framework',
                        default='timenet')
    parser.add_argument('--loss_sim_coeff',
                        type=float,
                        help='The max value of coefficient for'
                             ' the similarity measure of representations (Lambda Max)',
                        default=0.9)
    parser.add_argument('--loss_sim_coeff_start_value',
                        type=float,
                        help='The initial value of coefficient for'
                             ' the similarity measure of representations (Lambda 0)',
                        default=0.0)
    parser.add_argument('--loss_sim_coeff_increase_rate',
                        type=float,
                        help='Rate of increase for the similarity measure'
                             ' of representations during the training (tau)',
                        default=1.5)
    parser.add_argument('--loss_sim_coeff_val',
                        type=float,
                        help='The value of the coefficient for'
                             ' the similarity measure of representations'
                             ' to use during validation',
                        default=0.0)
    parser.add_argument('--loss_start_epoch_threshold',
                        type=int,
                        help='The epoch to start using the loss coefficient (psi_0)',
                        default=0)
    ...
    main_logic(dataset_index=args.dataset_index,
               ...
               loss_sim_coeff=args.loss_sim_coeff,
               loss_sim_coeff_start_value=args.loss_sim_coeff_start_value,
               loss_sim_coeff_increase_rate=args.loss_sim_coeff_increase_rate,
               loss_sim_coeff_val=args.loss_sim_coeff_val,
               loss_start_epoch_threshold=args.loss_start_epoch_threshold,
